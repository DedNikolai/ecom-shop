...
export default function CategoryPage() {
  ...
  const { data, isLoading } = useProducts({ page: 1, limit: LIMIT, categoryUrl: category });
  const cat = useCategoryByUrl(category)

  if (isLoading || cat.isPending)
    return (
      ...
      </h1>
    {
      ...
      <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-5">
        {data.items.map((product) => (
          <Link href={`${publicRoutes._CATEGORY}/${cat.data?.url}/${product.url}`} key={product.id}>
            <li 
